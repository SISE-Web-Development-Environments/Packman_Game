...
$(function () {
	...
	var error_keys = true;
	var error_upKey = true;
	var error_downKey = true;
	var error_rightKey = true;
	var error_leftKey = true;

	var error_numOfBalls = true;
	var error_timeOfGame= true;
	var error_numberOfMansters = true;

	var error_colors=true;
...
$("#formControlers").submit(function () {
	error_upKey = false;
	error_downKey = false;
	...
	if (error_upKey === false && error_downKey === false && error_rightKey === false && error_leftKey === false) {
		alert("setting done");
		//showLogin();
	...
	for (var i = 0; i < 19; i++) {
		...
		for (var j = 0; j < 14; j++) {
			if (
				...
				if (randomNum <= (1.35 * food_remain) / cnt) {
					food_remain--;
					board[i][j] = 1;
				} else if (randomNum < (1.35 * (pacman_remain + food_remain)) / cnt) {
					shape.i = i;
					shape.j = j;
					pacman_remain--;
					board[i][j] = 2;
